...
|  | 版本 | 日期 | 类型 | 说明 |
|---|------|------|------|------|
|  | **v0.9.0-beta.1** | 2026-02-25 | Feature | **Analysis详情页独立页面重构**<br>- 点击全局统计卡片可进入详情页（TrainingAnalysisDetailPage）<br>- 详情页从嵌套Tab改为独立完整页面<br>- 身体数据从卡片形式改为第1个Tab（其他3个Tab依次向后排列）<br>- Tab顺序：身体数据 -> 容量统计 -> 力量表现 -> 动作分析 |
